...
- Add `clean()` method to `Pet` and a `CLEAN` popup button in the UI; tests for cleaning live in `Tamagotchi/tests/test_cleanliness.py`.

New actions: `give_medicine()` and `MENU` popup
- `give_medicine()` heals +15 health, reduces happiness by -5, no effect if dead; tests in `Tamagotchi/tests/test_medicine.py`.
- Small-screen UI: a compact `MENU` toggle (top-right) opens a popup with `CLEAN` and `MED` actions to save screen space on 480x320 displays. Use `GameEngine.step()` in headless tests to simulate clicks (see `Tamagotchi/tests/test_ui.py`).
...
- Tests for evolution are in `Tamagotchi/tests/test_medicine.py`.

Balance & testing
- Gameplay tunables are exposed as constants at the top of `tamagotchi.py` (e.g., `HUNGER_DECAY_PER_HOUR`) so automated balance sweeps and tests can adjust them easily.
